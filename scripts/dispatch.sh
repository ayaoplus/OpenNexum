...
def load_contract():
    ...
    if yaml is not None:
        try:
            ...
            fail(f"Failed to parse contract YAML {contract_file}: {exc}")
    else:
        tokens = tokenize(raw)
        if not tokens:
            fail(f"Contract is empty: {contract_file}")
        data, index = parse_block(tokens, 0, tokens[0][0])
        if index != len(tokens):
            fail(f"Unsupported YAML structure in {contract_file}")

    if not isinstance(data, dict):
        fail(f"Contract root must be a mapping: {contract_file}")
    return data
